...
namespace vehicle
{
    ...
    double Vehicle::update(double timestep, double distanceToControlPoint) {
        ...
        const double brakingSpeedLimit = std::sqrt(std::max(
            0.0,
            2.0 * deceleration * std::max(0.0, distanceToControlPoint)));
        ...
        const double travelledDistance = std::min(
            std::max(0.0, distanceToControlPoint),
    ...
    double Vehicle::getCurrentSpeedKph() const { return currentSpeed * 3.6; };
