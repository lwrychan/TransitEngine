...
void core::World::assignVehicleToRoute(VehicleId vehicle, AbstractRouteId route)
{
    ...
    for (AbstractNodeId node : this->routes.get(route).getNodes())
    {
        const bool alreadyAdded = std::any_of(
            stops.begin(), stops.end(), [node](const vehicle::RouteStop& stop)
...
std::optional<core::PhysicalVehiclePose> core::World::getVehiclePose(VehicleId vehicle) const
{
    ...
    {
        const network::PhysicalCoordinate& start =
            getPhysicalNode(nodesOnRoute[index - 1]).getCoordinate();
        const network::PhysicalCoordinate& end =
            getPhysicalNode(nodesOnRoute[index]).getCoordinate();
        ...
        if (remainingDistance <= segmentLength || index + 1 == nodesOnRoute.size())
        {
            ...
            return PhysicalVehiclePose{
                .coordinate = {start.x + deltaX * fraction, start.y + deltaY * fraction,
                               start.z + (end.z - start.z) * fraction},
                .direction = {(deltaX / segmentLength) * currentVehicle.getRouteDirection(),
                              (deltaY / segmentLength) * currentVehicle.getRouteDirection(), 0.0}};
        }
        ...
        [&](VehicleId, vehicle::Vehicle& vehicle)
        {
            ...
            for (size_t nodeIndex = 0; nodeIndex < nodesOnRoute.size(); ++nodeIndex)
            {
                const auto stop = std::find_if(
                    ...
                    {
                        return candidate.node.id == node.id &&
                               candidate.node.generation == node.generation;
                    });
                ...
                if (distance > arrivalEpsilon && distance < nextControl.distance)
                {
                    nextControl = {.distance = distance,
                                   ...
                                   .reversesAtEnd =
                                       !routeClosed &&
                                       (direction > 0 ? nodeIndex + 1 == nodesOnRoute.size()
                                                      : nodeIndex == 0)};
                }
            }
            ...
            {
                nextControl = {.distance =
                                   direction > 0 ? routeLength - routeProgress : routeProgress,
            ...
            if (routeClosed)
            {
                vehicle.setRouteProgressMeters(
                    std::fmod(vehicle.getRouteProgressMeters(), routeLength));
            }
        });
    this->tickCount++;
}
