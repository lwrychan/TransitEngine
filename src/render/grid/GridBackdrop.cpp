...
void drawGridBackdrop(const GridCamera& camera, const GridCanvas& canvas)
{
    ...
    const network::PhysicalCoordinate topLeft =
        camera.screenToWorld(canvas.position, canvas.position, canvas.size);
    const network::PhysicalCoordinate bottomRight =
        camera.screenToWorld(maximum, canvas.position, canvas.size);
    const double minimumX = std::min(topLeft.x, bottomRight.x);
    const double maximumX = std::max(topLeft.x, bottomRight.x);
    const double minimumY = std::min(topLeft.y, bottomRight.y);
    const double maximumY = std::max(topLeft.y, bottomRight.y);
    ...
    {
        const float screenX = camera.worldToScreen({x, 0.0, 0.0}, canvas.position, canvas.size).x;
        const bool axis = std::abs(x) < step * 0.001;
        const bool major = std::abs(std::fmod(std::abs(x), majorStep)) < step * 0.001;
        canvas.drawList->AddLine({screenX, canvas.position.y}, {screenX, maximum.y},
                                 axis    ? IM_COL32(80, 95, 110, 150)
                                 : major ? IM_COL32(112, 128, 145, 85)
                                         : IM_COL32(142, 157, 172, 34));
    }
    for (double y = firstY; y <= maximumY + step * 0.5; y += step)
    {
        const float screenY = camera.worldToScreen({0.0, y, 0.0}, canvas.position, canvas.size).y;
        const bool axis = std::abs(y) < step * 0.001;
        const bool major = std::abs(std::fmod(std::abs(y), majorStep)) < step * 0.001;
        canvas.drawList->AddLine({canvas.position.x, screenY}, {maximum.x, screenY},
                                 axis    ? IM_COL32(80, 95, 110, 150)
                                 : major ? IM_COL32(112, 128, 145, 85)
                                         : IM_COL32(142, 157, 172, 34));
    }
    ...
    canvas.drawList->AddRectFilled({barStart.x - 7.0f, barStart.y - labelSize.y - 12.0f},
                                   {barEnd.x + 7.0f, barStart.y + 7.0f},
                                   IM_COL32(246, 248, 250, 225), 3.0f);
    ...
    canvas.drawList->AddLine({barEnd.x, barEnd.y - 5.0f}, {barEnd.x, barEnd.y + 5.0f},
                             IM_COL32(20, 30, 40, 255), 2.0f);
    canvas.drawList->AddText({barStart.x, barStart.y - labelSize.y - 6.0f},
                             IM_COL32(20, 30, 40, 255), label);
