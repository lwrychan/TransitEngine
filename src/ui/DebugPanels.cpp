...
namespace ui
{
    namespace
    {
        ...
        PanelLayout getPanelLayout() {
            ...
            const float contentHeight = std::max(
                180.0f,
                viewport->WorkSize.y - margin * 2.0f - toolbarOffset);
    ...
    namespace
    {
        ...
        bool isStopSelected(
            const std::vector<vehicle::RouteStop>& stops,
            AbstractNodeId node) {
            return std::find_if(stops.begin(), stops.end(),
                [node](const vehicle::RouteStop& selected) { return sameNode(selected.node, node); })
                != stops.end();
        }
        ...
        std::vector<vehicle::RouteStop> validStopsForRoute(
            const std::vector<vehicle::RouteStop>& stops) {
            std::vector<vehicle::RouteStop> validStops;
            for (AbstractNodeId node : routeNodes) {
                const auto stop = std::find_if(stops.begin(), stops.end(),
                    [node](const vehicle::RouteStop& candidate) {
        ...
        bool drawCoordinateInput(const char* label, char* buffer, double& coordinate) {
            ...
            std::string errorReason;
            const bool isValidBeforeInput = parseCoordinateInput(
                buffer, parsedValue, errorReason);

            ImGui::PushStyleColor(
                ImGuiCol_FrameBg,
                coordinateInputActive[inputIndex]
            ...
            const bool wasSubmitted = ImGui::InputText(
                label,
                buffer,
                32,
            ...
            const bool shouldCommit =
                wasSubmitted || ImGui::IsItemDeactivatedAfterEdit();
    ...
    bool isModalOpen() {
        return showNodeCreationModal
            || showVehicleCreationModal
            || showNodeMoveConfirmation
    ...
    void drawRouteControls(core::World& world) {
        ...
        ImGui::SetNextWindowPos(ImVec2(
            viewport->WorkPos.x,
            viewport->WorkPos.y + viewport->WorkSize.y - height), ImGuiCond_Always);
        ...
        ImGui::SameLine();
        const bool routeIsClosed = routeNodes.size() >= 3
            && sameNode(routeNodes.front(), routeNodes.back());
        if (routeNodes.size() >= 2) {
            if (routeIsClosed) {
        ...
        ImGui::SameLine();
        if (whiteButton("Clear")) routeNodes.clear();
        ImGui::End();

        if (showRouteSaveModal) ImGui::OpenPopup("Save Route");
        if (ImGui::BeginPopupModal("Save Route", nullptr, ImGuiWindowFlags_AlwaysAutoResize)) {
            ...
            world.forEachVehicle([&](VehicleId vehicleId, vehicle::Vehicle& vehicle) {
                ...
                if (ImGui::Checkbox(vehicle.getDisplayName().c_str(), &selected)) {
                    if (selected) {
                        ...
                        routeVehicles.erase(std::remove_if(routeVehicles.begin(), routeVehicles.end(),
                    ...
                    for (size_t nodeIndex = 0; nodeIndex < routeNodes.size(); ++nodeIndex) {
                        ...
                        if (ImGui::Checkbox(node.getName().c_str(), &stopsHere)) {
                            if (stopsHere) {
                                ...
                                assignment->stops.erase(std::remove_if(
                                    assignment->stops.begin(), assignment->stops.end(),
            ...
            for (size_t index = 0; index < routeNodes.size(); ++index) {
                ...
                ImGui::Text("%zu. %s (%.0f, %.0f, %.0f m)", index + 1,
                    node.getName().c_str(), coordinate.x, coordinate.y, coordinate.z);
                ImGui::SameLine();
                if (index > 0 && whiteSmallButton("Up")) std::swap(routeNodes[index], routeNodes[index - 1]);
                ImGui::SameLine();
                if (index + 1 < routeNodes.size() && whiteSmallButton("Down")) std::swap(routeNodes[index], routeNodes[index + 1]);
                ImGui::SameLine();
                if (whiteSmallButton("Remove")) routeNodes.erase(routeNodes.begin() + index);
                ImGui::PopID();
            }
    ...
    void drawNodeCreationModal(core::World& world) {
        ...
        if (ImGui::BeginPopupModal("Node Editor", nullptr, ImGuiWindowFlags_AlwaysAutoResize)) {
            ...
            popTextInputStyle();
            const bool xValid = drawCoordinateInput(
                "X", nodeCreationCoordinateInputs[0], nodeCreationCoordinate.x);
            const bool yValid = drawCoordinateInput(
                "Y", nodeCreationCoordinateInputs[1], nodeCreationCoordinate.y);
            const bool zValid = drawCoordinateInput(
                "Z", nodeCreationCoordinateInputs[2], nodeCreationCoordinate.z);
    ...
    void drawDeleteConfirmation(core::World& world) {
        ...
        const bool itemExists = deletingNode
            ? editingNode.has_value()
            : routePendingDeletion.has_value();
    ...
    void drawSimulationControls(Simulation& simulation) {
        ...
        ImGui::SameLine();
        if (whiteButton("0.25x")) { simulation.setSpeedMultiplier(0.25); }
        ImGui::SameLine();
        if (whiteButton("1x")) { simulation.setSpeedMultiplier(1.0); }
        ImGui::SameLine();
        if (whiteButton("2x")) { simulation.setSpeedMultiplier(2.0); }
        ImGui::SameLine();
        if (whiteButton("4x")) { simulation.setSpeedMultiplier(4.0); }
    ...
    void drawEditorPanel(Simulation& simulation) {
        ...
        if (world.getActiveRoute().has_value()) {
            ...
            ImGui::Text("Coordinate: (%.0f, %.0f, %.0f) m",
    ...
    void drawNodeTab(Simulation& simulation) {
        ...
        if (ImGui::BeginTable("Nodes", 5, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg
            | ImGuiTableFlags_ScrollX,
