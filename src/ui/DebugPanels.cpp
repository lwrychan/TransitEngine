...
namespace
{
  ...
  PanelLayout getPanelLayout(bool showDebugPanels)
  {
    const ImGuiViewport* viewport = ImGui::GetMainViewport();
    constexpr float panelGap = render::editorlayout::panelGap;
    ...
    const render::editorlayout::ContentBounds bounds =
        render::editorlayout::contentBounds(*viewport, ImGui::GetFrameHeight());
    const float leftX = bounds.left;
    const float rightX = bounds.right - rightWidth;
    const float bottomY = bounds.bottom;
    const float topY = bounds.top;
    const float contentHeight = std::max(1.0f, bottomY - topY);
    ...
    const float inspectorY = topY + simulationHeight + panelGap;
    const float inspectorHeight =
        std::min(vehicleHeight, std::max(1.0f, bottomY - inspectorY - panelGap) * 0.55f);
    const float editorY = inspectorY + inspectorHeight + panelGap;
    const auto heightToBottom = [bottomY](float panelTop)
    { return std::max(1.0f, bottomY - panelTop); };

    if (!showDebugPanels)
    {
      return {.simulationPosition = ImVec2(leftX, topY),
              .simulationSize = ImVec2(leftWidth, simulationHeight),
              .inspectorPosition = ImVec2(leftX, inspectorY),
              .inspectorSize = ImVec2(leftWidth, inspectorHeight),
              .abstractNetworkPosition = ImVec2(leftX, editorY),
              .abstractNetworkSize = ImVec2(leftWidth, heightToBottom(editorY))};
    }
    ...
    return {.simulationPosition = ImVec2(leftX, topY),
            ...
            .logSize = ImVec2(leftWidth, heightToBottom(leftLogY)),
            .inspectorPosition = ImVec2(rightX, topY),
            .inspectorSize = ImVec2(rightWidth, vehicleHeight),
            ...
            .abstractNetworkSize = ImVec2(rightWidth, heightToBottom(rightAbstractY))};
  }
  ...
  struct LatencyDebugSample
  {
    double lastUpdatedSeconds = 0.0;
    double renderLatencyMs = 0.0;
    double simulationLatencyMs = 0.0;
    double overallLatencyMs = 0.0;
    bool hasValue = false;
  };

  void updateLatencyDebugSample(LatencyDebugSample& sample, const Simulation& simulation,
                                double sampleIntervalSeconds)
  {
    const double now = ImGui::GetTime();
    if (sample.hasValue && now - sample.lastUpdatedSeconds < sampleIntervalSeconds)
    {
      return;
    }

    sample.lastUpdatedSeconds = now;
    sample.renderLatencyMs = simulation.getLastRenderLatencyMs();
    sample.simulationLatencyMs = simulation.getLastSimulationLatencyMs();
    sample.overallLatencyMs = simulation.getLastOverallLatencyMs();
...
namespace
{
  ...
  bool isRouteVehicleSelected(VehicleId vehicle)
  {
    return std::find_if(routeVehicles.begin(), routeVehicles.end(),
                        ...
                        { return sameVehicle(selected.vehicle, vehicle); }) != routeVehicles.end();
  }
  ...
  bool drawCoordinateInput(const char* label, char* buffer, double& coordinate,
                           double planarGridHalfExtent)
  {
    const int inputIndex = label[0] == 'X' ? 0 : (label[0] == 'Y' ? 1 : 2);
    const bool hasPlanarGridBound = inputIndex < 2;
    const auto isValidCoordinateInput =
        [hasPlanarGridBound, planarGridHalfExtent](const char* input, double& meters,
                                                   std::string& errorReason)
    {
      if (!parseCoordinateInput(input, meters, errorReason))
      {
        return false;
      }
      if (hasPlanarGridBound && std::abs(meters) > planarGridHalfExtent)
      {
        char rangeMessage[96];
        std::snprintf(rangeMessage, sizeof(rangeMessage),
                      "X and Y must be between %.0f m and %.0f m.", -planarGridHalfExtent,
                      planarGridHalfExtent);
        errorReason = rangeMessage;
        return false;
      }
      return true;
    };
    ...
    std::string errorReason;
    const bool isValidBeforeInput = isValidCoordinateInput(buffer, parsedValue, errorReason);
    ...
    const bool isValid = isValidCoordinateInput(buffer, parsedValue, errorReason);
  ...
  struct VehicleCreationValidation
  {
    bool hasDisplayName = false;
    bool hasValidMaxSpeed = false;
    bool hasValidAcceleration = false;
    bool hasValidDeceleration = false;
    bool hasValidPassengerCapacity = false;

    bool isValid() const
    {
      return hasDisplayName && hasValidMaxSpeed && hasValidAcceleration && hasValidDeceleration &&
             hasValidPassengerCapacity;
    }
  };

  bool hasNonWhitespaceText(const char* text)
  {
    return std::any_of(text, text + std::strlen(text),
                       [](unsigned char character) { return !std::isspace(character); });
  }

  VehicleCreationValidation validateVehicleCreationDraft(const VehicleCreationDraft& draft)
  {
    return {.hasDisplayName = hasNonWhitespaceText(draft.displayName),
            .hasValidMaxSpeed = std::isfinite(draft.maxSpeedKph) && draft.maxSpeedKph > 0.0f,
            .hasValidAcceleration = std::isfinite(draft.accelerationKphPerSecond) &&
                                    draft.accelerationKphPerSecond > 0.0f,
            .hasValidDeceleration = std::isfinite(draft.decelerationKphPerSecond) &&
                                    draft.decelerationKphPerSecond > 0.0f,
            .hasValidPassengerCapacity = draft.passengerCapacity > 0};
  }

  void pushValidatedInputStyle(bool isValid)
  {
    pushTextInputStyle();
    if (!isValid)
    {
  ...
  void popValidatedInputStyle(bool isValid)
  {
    if (!isValid)
    {
      ImGui::PopStyleVar();
  ...
  void drawValidationError(bool isValid, const char* error)
  {
    if (!isValid)
    {
      ImGui::TextColored(ImVec4(0.85f, 0.12f, 0.12f, 1.0f), "%s", error);
    }
  }

  void drawVehicleFloatInput(const char* label, float& value, bool isValid, const char* error,
                             float step, float stepFast)
  {
    pushValidatedInputStyle(isValid);
    if (ImGui::InputFloat(label, &value, step, stepFast, "%.1f") &&
        (!std::isfinite(value) || value < 0.0f))
    {
      value = 0.0f;
    }
    popValidatedInputStyle(isValid);
    drawValidationError(isValid, error);
  }
  ...
  {
    const VehicleCreationValidation validation = validateVehicleCreationDraft(draft);
    if (!validation.hasDisplayName)
    {
    ...
    {
      LogBuffer::instance().add("Vehicle creation failed: numeric properties must be finite "
  ...
  void drawVehicleCreationModal(core::World& world)
  {
    ...
    if (ImGui::BeginPopupModal("Create Vehicle", nullptr, ImGuiWindowFlags_AlwaysAutoResize))
    {
      ...
      pushValidatedInputStyle(validation.hasDisplayName);
      ImGui::InputText("Display Name", draft.displayName, sizeof(draft.displayName));
      popValidatedInputStyle(validation.hasDisplayName);
      drawValidationError(validation.hasDisplayName, "Enter a display name.");
      drawVehicleFloatInput("Max Speed (km/h)", draft.maxSpeedKph, validation.hasValidMaxSpeed,
                            "Enter a finite speed greater than zero.", 1.0f, 10.0f);
      drawVehicleFloatInput("Acceleration (km/h/s)", draft.accelerationKphPerSecond,
                            validation.hasValidAcceleration,
                            "Enter a finite acceleration greater than zero.", 1.0f, 5.0f);
      drawVehicleFloatInput("Deceleration (km/h/s)", draft.decelerationKphPerSecond,
                            validation.hasValidDeceleration,
                            "Enter a finite deceleration greater than zero.", 1.0f, 5.0f);
      pushValidatedInputStyle(validation.hasValidPassengerCapacity);
      if (ImGui::InputInt("Passenger Capacity", &draft.passengerCapacity, 1, 10) &&
          draft.passengerCapacity < 0)
      {
        draft.passengerCapacity = 0;
      }
      popValidatedInputStyle(validation.hasValidPassengerCapacity);
      drawValidationError(validation.hasValidPassengerCapacity,
                          "Enter a passenger capacity greater than zero.");
...
void drawRouteControls(core::World& world, render::Tool activeTool)
{
  ...
  const render::editorlayout::ContentBounds bounds =
      render::editorlayout::contentBounds(*viewport, ImGui::GetFrameHeight());
  constexpr float height = render::editorlayout::routeControlsHeight;
  ImGui::SetNextWindowPos(ImVec2(bounds.left, viewport->WorkPos.y + viewport->WorkSize.y - height),
                          ImGuiCond_Always);
  ImGui::SetNextWindowSize(ImVec2(bounds.right - bounds.left, height), ImGuiCond_Always);
  ImGui::Begin("Route Selection", nullptr,
               ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoTitleBar |
                   ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoBringToFrontOnFocus |
                   ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse);
  ImGui::TextDisabled("Active tool: %s", render::toolName(activeTool));
  if (activeTool == render::Tool::Pointer)
  {
    ImGui::SameLine();
    ImGui::TextDisabled("·");
    ImGui::SameLine();
    ImGui::TextDisabled("Drag to pan using LEFT ALT or Middle Mouse");
  }
  else if (activeTool == render::Tool::Route)
  {
    ImGui::SameLine();
    ImGui::TextDisabled("·");
    ImGui::SameLine();
  ...
  if (activeTool == render::Tool::Geometry && !world.getActiveRoute().has_value())
  {
    ImGui::SameLine();
    ImGui::TextDisabled("·");
    ImGui::SameLine();
    ImGui::TextColored(ImVec4(1.0f, 0.72f, 0.18f, 1.0f),
                       "To use the Geometry tool, first select a Route.");
  }
  else if (activeTool == render::Tool::Route)
  {
    ImGui::SameLine();
  ...
  if (ImGui::BeginPopupModal("Save Route", nullptr, ImGuiWindowFlags_AlwaysAutoResize))
  {
    ...
    world.forEachVehicle(
        [&](VehicleId vehicleId, vehicle::Vehicle& vehicle)
        {
          ...
          if (ImGui::Checkbox(vehicle.getDisplayName().c_str(), &selected))
          {
            ...
            {
              routeVehicles.erase(
                  std::remove_if(routeVehicles.begin(), routeVehicles.end(),
          ...
          if (selected)
          {
            ...
            for (size_t nodeIndex = 0; nodeIndex < routeNodes.size(); ++nodeIndex)
            {
              ...
              if (ImGui::Checkbox(node.getName().c_str(), &stopsHere))
              {
                ...
                else
                {
                  assignment->stops.erase(
                      std::remove_if(assignment->stops.begin(), assignment->stops.end(),
              ...
              if (stopsHere)
              {
                auto stop = std::find_if(assignment->stops.begin(), assignment->stops.end(),
                                         [nodeId](const vehicle::RouteStop& selectedStop)
                                         { return sameNode(selectedStop.node, nodeId); });
                ImGui::SameLine();
    ...
    for (size_t index = 0; index < routeNodes.size(); ++index)
    {
      ...
      const network::AbstractNode& node = world.getNode(nodeId);
      const network::PhysicalCoordinate& coordinate = world.getPhysicalNode(nodeId).getCoordinate();
      ImGui::PushID(static_cast<int>(index));
      ImGui::Text("%zu. %s (%.0f, %.0f, %.0f m)", index + 1, node.getName().c_str(), coordinate.x,
      ...
      if (routeName[0] != '\0' && routeNodes.size() >= 2)
      {
        ...
        world.forEachVehicle(
            [&](VehicleId vehicleId, vehicle::Vehicle& vehicle)
            {
              ...
              const std::optional<AbstractRouteId>& assignedRoute = vehicle.getAssignedRoute();
              const bool assignedToSavedRoute = assignedRoute.has_value() &&
                                                assignedRoute->id == savedRoute.id &&
              ...
              if (selectedForRoute)
              {
                ...
                world.setVehicleRouteStops(vehicleId, validStopsForRoute(assignment->stops));
              }
  ...
  if (ImGui::BeginPopupModal("Confirm Node Move", nullptr, ImGuiWindowFlags_AlwaysAutoResize))
  {
    ...
    if (whiteButton("Move", ImVec2(120.0f, 0.0f)))
    {
      if (movingNode.has_value())
      {
        ...
        {
          LogBuffer::instance().add("Node Editor: A node already exists at that coordinate.");
        }
      }
...
void drawNodeCreationModal(Simulation& simulation)
{
  core::World& world = simulation.getWorld();
  const double planarGridHalfExtent =
      std::sqrt(
          std::max(0.0, simulation.getConfig().renderingConfig.maximumGridZoomAreaSquareMeters)) /
      2.0;
  ...
  if (ImGui::BeginPopupModal("Node Editor", nullptr, ImGuiWindowFlags_AlwaysAutoResize))
  {
    ...
    popTextInputStyle();
    const bool xValid = drawCoordinateInput("X", nodeCreationCoordinateInputs[0],
                                            nodeCreationCoordinate.x, planarGridHalfExtent);
    const bool yValid = drawCoordinateInput("Y", nodeCreationCoordinateInputs[1],
                                            nodeCreationCoordinate.y, planarGridHalfExtent);
    const bool zValid = drawCoordinateInput("Z", nodeCreationCoordinateInputs[2],
                                            nodeCreationCoordinate.z, planarGridHalfExtent);
    ...
    if (whiteButton("Confirm", ImVec2(120, 0)))
    {
      ...
      const bool updated = wasEditing ? world.updateNode(*editingNode, nodeCreationType,
      ...
      else
      {
        ...
        ImGui::CloseCurrentPopup();
        editingNode.reset();
        ui::LogBuffer::instance().add("Node Creator: A node already exists at this coordinate.");
      }
    }
    if (!xValid || !yValid || !zValid)
    {
...
void drawDeleteConfirmation(core::World& world)
{
  ...
  const bool itemExists = deletingNode ? editingNode.has_value() : routePendingDeletion.has_value();
  ...
  if (ImGui::BeginPopupModal(title, nullptr, ImGuiWindowFlags_AlwaysAutoResize))
  {
    ...
    if (whiteButton("Delete", ImVec2(120, 0)))
    {
      ...
      else if (itemExists)
      {
        ...
        routeVehicles.clear();
        LogBuffer::instance().add("Route Editor: Deleted route and unassigned its vehicles.");
      }
      editingNode.reset();
...
{
  const PanelLayout layout = getPanelLayout(showDebugPanels);
  applyPanelLayout(layout.simulationPosition, layout.simulationSize);
  ImGui::Begin("Simulation Controls", nullptr, ResponsivePanelFlags | ImGuiWindowFlags_NoScrollbar);
  ...
  const float resetButtonWidth = ImGui::CalcTextSize("1x").x + style.FramePadding.x * 2.0f;
  const float speedSliderWidth =
      std::max(1.0f, ImGui::GetContentRegionAvail().x - resetButtonWidth - style.ItemSpacing.x);
  ImGui::SetNextItemWidth(speedSliderWidth);
  if (ImGui::SliderFloat("##SpeedMultiplier", &speed, 0.25f, 200.0f, "%.2fx",
  ...
  core::World& world = simulation.getWorld();
  ImGui::Text("%.2f s · (%llu ticks)", simulation.getSimTime(),
              static_cast<unsigned long long>(world.getTickCount()));
  ImGui::Text("Status: %s",
...
{
  const PanelLayout layout = getPanelLayout(true);
  ...
  const double sampleIntervalSeconds =
      std::max(0.001, simulation.getConfig().uiConfig.latencyDebugSampleIntervalSeconds);

  static LatencyDebugSample sample;
  updateLatencyDebugSample(sample, simulation, sampleIntervalSeconds);

  const double renderLatencyMs = sample.renderLatencyMs;
  const double simulationLatencyMs = sample.simulationLatencyMs;
  const double overallLatencyMs = sample.overallLatencyMs;
  ...
  if (ImGui::Checkbox("DEBUG_CLOCK console logging", &debugClock))
  {
    ...
    LogBuffer::instance().add(std::string("DEBUG_CLOCK ") + (debugClock ? "enabled" : "disabled") +
                              ".");
  }
...
void drawEditorPanel(Simulation& simulation, bool showDebugPanels)
{
  ...
  if (world.getActiveRoute().has_value())
  {
    ...
    render::grid::drawGridGeometryControls(world);
  }
...
{
  const PanelLayout layout = getPanelLayout(showDebugPanels);
  applyPanelLayout(layout.inspectorPosition, layout.inspectorSize);
  ImGui::Begin("Inspector", nullptr, ResponsivePanelFlags);
...
void drawEditorDialogs(Simulation& simulation)
{
  ...
  drawNodeCreationModal(simulation);
