...
namespace network
{
    ...
    public:
        ...
        const std::vector<AbstractNodeId>& getNodes() const { return nodes; }
        const RouteColor& getColor() const { return color; }
        void addSegment(AbstractSegmentId segmentId) { segments.push_back(segmentId); }
        void addNode(AbstractNodeId nodeId) { nodes.push_back(nodeId); }
        void setName(std::string newName) { name = std::move(newName); }
        void setNodes(std::vector<AbstractNodeId> newNodes) { nodes = std::move(newNodes); }
        void setColor(RouteColor newColor) { color = newColor; }
